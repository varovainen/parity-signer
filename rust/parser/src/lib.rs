...
pub mod cards;
use cards::{ParserCard, Specialty};
mod decoding_older;
...
pub fn parse_and_display_set(
    ...
    for (i, x) in extensions_cards.iter().enumerate() {
        ...
        extensions.push_str(&x.card.show_no_docs(x.indent, short_specs));
    }

    // try parsing method
    let method = match parse_method(&mut method_data, &metadata_bundle) {
        Ok(method_cards) => {
            ...
            for (i, x) in method_cards.iter().enumerate() {
                ...
                collect.push_str(&x.card.show_no_docs(x.indent, short_specs));
            }
            collect
        }
        Err(e) => e.show(),
...
pub fn decode_blob_as_type(data: &mut Vec<u8>, ty: &Type<PortableForm>, meta_v14: &RuntimeMetadataV14) -> Result<Vec<OutputCard>, ParserError> {
